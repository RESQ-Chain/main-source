...
// Copyright (c) 2018 hav0k, Renesis Developers
...
#if defined(NDEBUG)
# error "wallet cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Wallet Signed Message:\n";
...
// LTME Reward Structure
int64_t GetProofOfWorkReward(int64_t nFees)
{   
    int64_t nSubsidy;
    if (pindexBest->nHeight < 100) {
        nSubsidy = 350000 * COIN; //Premine total 35000000 RESS
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 5001) {
        nSubsidy = 500 * COIN; //Starting block 100 and ending at block 5000 mining will generate 2,450,000 at 500 RESS each block
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 150001) { // This is where we fork the Renesis algo first
        nSubsidy = 800 * COIN; //Starting block 5001 and ending block 150000 mining will generate 116,000,000 at 800 RESS each block
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 300001) { // This is where we will fork the Renesis algo again
        nSubsidy = 150 * COIN;// Starting block 150001 and ending at block 300000 mining will generate  22,500,000 at 150 RESS each block
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 500001) { // This is where we will fork the Renesis algo again
        nSubsidy = 300 * COIN; //Starting block 300001 ending block 500000 mining will generate 60,000,000 at 300 RESS per block
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 1000001) { // This is where we will fork the Renesis algo again
        nSubsidy = 100 * COIN; //Starting block 500001 till block till block 1000000 mining will generate 100,000,000 at 100 RESS per block
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 1070251) {
        nSubsidy = 200 * COIN; //Starting block 1000001 Renesis will turn Hybrid giving 200 RESS per block and 10% PoS reward per year.
        return nSubsidy + nFees;
    } else {
        nSubsidy = 0 * COIN;// After left over PoW is completed, coin will remain 0 RESS per mining block and 10% PoS reward per year.
        return nSubsidy + nFees; // After PoW is completed, Renesis algo will not be forked based on blocks.
    }
...
#ifdef ENABLE_WALLET
// wallet: attempt to generate suitable proof-of-stake
