...
//
// A clean exit happens when StartShutdown() or the RESSERM
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: renesis.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: renesisd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 9775 or testnet: 19775)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 97744 or testnet: 19774)") + "\n";
...
bool AppInit2(boost::thread_group& threadGroup)
{
    ...
    // Minimum supported OS versions: WRESSP SP3, WinVista >= SP1, Win Server 2008
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Wallet is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. daemon is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Wallet version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            {
                strErrors << _("Wallet needed to be rewritten: restart wallet to complete") << "\n";
